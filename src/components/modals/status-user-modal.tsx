...
const StatusUserModal = () => {
    ...
    React.useEffect(() => {
        if (member) {
            setStatusText(member.user.statusText || "");
            setEmoji(member.user.statusEmoji || "");
        }
    }, [member]);

    if (!isModalOpen || !clientId) return;

    const handleEmojiSelected = (unified: string) => {
        const url = `https://cdn.jsdelivr.net/npm/emoji-datasource-google/img/google/64/${unified}.png`;
        setEmoji(url);
    };
            ...
            <DialogContent className="w-[520px] bg-background shadow-[0_2px_10px_0_hsl(0_calc(1_*_0%)_0%_/_0.2)] p-0 overflow-hidden border-0 !rounded-[6px]">
                ...
                <div className="m-[12px_20px_8px_20px]">
                    <div className="flex items-center relative focus-within:!shadow-focus-border h-[42px] rounded-[4px] focus-within:!rounded-[6px] overflow-hidden w-full focus-within:!border-transparent border border-foreground/50">
                        <div className="w-[42px] h-full flex items-center justify-center">
                            <EmojiPicker
                                onChange={(emoji) => handleEmojiSelected(emoji.unified)}
                                side="bottom"
                                emojiComponent={() => (
                                    <div>
                                        {emoji && (
                                            <div className="w-[24px] h-[24px] overflow-hidden relative cursor-pointer">
                                                <Image src={emoji} alt="Status" fill className="w-full h-full object-cover" />
                                            </div>
                                        )}

                                        {!emoji && (
                                            <div className="p-0 text-muted-foreground w-full h-full hover:bg-transparent bg-transparent rounded-none cursor-pointer">
                                                <EmojiIcon size={24} />
                                            </div>
                                        )}
                                    </div>
                        ...
                        <input type="text" value={statusText} onChange={(v) => setStatusText(v.target.value)} autoComplete="off" className="focus:outline-none focus:border-none w-full h-full border-none p-[9px_0] text-[18px] leading-[22px] placeholder:text-[#8D8D8D]" placeholder="What is your status?" />
                    </div>
                ...
                <DialogFooter className="p-[24px_28px]">
                    <div className="ml-auto">
                        ...
                        <Button onClick={handleSave} disabled={isLoading} variant={"success"} className={cn("ml-2 min-w-[80px] text-[15px] h-[36px] rounded-[8px] p-[0_12px]", {})}>
                            Save
                        </Button>
                    </div>
                </DialogFooter>
