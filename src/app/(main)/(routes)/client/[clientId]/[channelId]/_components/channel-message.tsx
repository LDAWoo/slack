...
const ChannelMessage = ({ user, member, channelId, apiUrl, socketUrl, socketQuery, paramValue, paramKey }: IChannelMessage) => {
    ...
    const groupedMessages = (messageArray: MessageWithMemberWithUser[]) => {
    ...
    return (
        <div ref={topRef} className="flex flex-col py-4 overflow-y-auto h-full p-0 scrollbarCustom">
            {!hasNextPage && <div className="flex-1"></div>}
            {!hasNextPage && <ChannelWelcome />}
            {hasNextPage && (
                            ...
                            Object.entries(groupedMessages(group.messages)).map(([dateKey, messages], index) => (
                                <div key={index}>
                                    ...
                                    {messages.map((message, index) => {
                                        const prevMessage = messages[index - 1] as MessageWithMemberWithUser;
